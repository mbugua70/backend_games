...
import { Event, EventDocument, GameType } from "../models/Event";
...
// Event codes are globally unique, not per-organization: the public mobile
// client looks an event up by code alone (entered in-app), with no
// organization context to disambiguate against.
...
// Public lookup key for the mobile client - no organizationId filter,
// since the client has no admin session to scope by.
...
// Gates the public write actions (registration, starting/completing a
// session) - GET /events/:code deliberately skips this so the client can
// still fetch the event and show a "not live" message instead of a bare 404.
...
// Deletes only the Event document - branding/config/players/sessions that
...
// Called by gameConfig/registrationConfig/leaderboardConfig services after
// a successful update, so a client can detect it needs to re-download
// config. Deliberately NOT called for branding changes (cosmetic only, not
// one of the settings the spec ties config_version to).
export const bumpConfigVersion = async (eventId: string): Promise<void> => {
  await Event.findByIdAndUpdate(eventId, { $inc: { configVersion: 1 } });
};
